...
- [x] **🚨 Vision model replaced: Nemotron-3-Nano Omni 30B Reasoning.** _(PR #31, 2026-05-02, session bd2c18549fd0423cbb3b7d36834185a8)._ User tested `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning` directly on NVIDIA's API and confirmed it correctly identifies handwritten math as differential equations (not chemistry/integrals like previous models). **Changes:**
  1. **Sole vision model** — replaced the multi-provider fallback chain (Llama-3.2-11B → Llama-3.2-90B → Gemini 2.0 Flash → Tesseract.js) with a single call to `nvidia/nemotron-3-nano-omni-30b-a3b-reasoning`. Env-var-overridable via `NVIDIA_VISION_MODEL`.
  2. **Removed Gemini and Tesseract.js** — per user request. No `GEMINI_API_KEY` needed anymore. Only `NVIDIA_API_KEY` is required for vision.
  3. **Reasoning model parameters** — added `chat_template_kwargs: { enable_thinking: false }` and `reasoning_budget: 16384` for reasoning-class models.
  4. **Increased limits** — `max_tokens` 1024 → 4096, timeout 40s → 60s, `temperature` 0 → 0.2, `top_p` 1 → 0.95.
  5. **Unconditional structured logging** (from PR #30) — `console.log` at every stage so Vercel function logs always show which model ran and what it returned.
  **Local test result:** OCR returned `\((D-2D^{3}+D^{2}y)y=x^{3}\)` — correctly identifies D-operator differential equation (vs previous models returning chemistry/integral nonsense). Superscript `4` is still tricky on rotated handwriting but the solver now gets the right problem domain. _Files:_ `src/lib/vision.ts`.
